...
setFavorite = async () => {
  const res = await fetch("http://localhost:3001/favorites", {
    ...
    body: JSON.stringify({city: this.state.cityInput, country: this.state.countryInput})
  })

  const json = await res.json();
  this.setState({
    ...this.state,
    favorites: [...this.state.favorites, json]
  })
}
...
componentDidMount() {
  fetch("http://localhost:3001/favorites")
    .then(res => res.json())
    .then((json) => this.setState({...this.state, favorites: json}));
}
          ...
          <div className="form-container">
            ...
            <div className="favorites">
              {this.state.favorites.map(favorite => {
                ...
                <button onClick={() => this.removeFavorite(favorite.id)}>remove favorite</button>
                </div>
        )})}
            </div>
          </div>
